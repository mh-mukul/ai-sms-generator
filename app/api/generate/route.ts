...
export async function POST(request: NextRequest) {
  try {
    ...
    if (baseUrl) {
      ...
      try {
        ...
        if (apiError instanceof Error) {
          ...
          // If environment is development, throw the error to avoid using mock data
          if (process.env.NODE_ENV === 'development') {
            // Return error details in development for easier debugging
    ...
    return NextResponse.json({ status: "error", message: "Failed to generate content" }, { status: 500 })
  }
}
